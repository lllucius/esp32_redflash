...
#define MOUNT_POINT "/redflash"
#define VOLUME_LABEL "external"
#define OPENFILES 4

// Set to the partition name if using internal flash,
// else comment to use external flash
//#define CONFIG_REDFS_PARTITION_LABEL  "littlefs"

static ExtFlash extflash;
static RedFlash redflash;
...
static void test_redflash_setup(int openfiles)
{
    ...
    TST(redflash.init(&red_cfg) == ESP_OK, "RedFlash initialization failed");
}

static void test_redflash_teardown()
{
    redflash.term();
}
...
{
    test_extflash_setup();
    test_redflash_setup(openfiles);
}

static void test_teardown()
{
    test_redflash_teardown();
    test_extflash_teardown();
}
...
{
    test_setup(OPENFILES);
    test_red_create_file_with_text(MOUNT_POINT "/hello.txt", red_test_hello_str);
    test_teardown();
}
...
TEST_CASE(can_read, "can read file", "[fatfs][wear_levelling]")
{
    test_setup(OPENFILES);
    test_red_create_file_with_text(MOUNT_POINT "/hello.txt", red_test_hello_str);
    test_red_read_file(MOUNT_POINT "/hello.txt");
    test_teardown();
}
...
TEST_CASE(can_open_max, "can open maximum number of files", "[fatfs][wear_levelling]")
{
    ...
    test_red_open_max_files(MOUNT_POINT "/f", max_files);
    test_teardown();
}
...
{
    test_setup(OPENFILES);
    test_red_overwrite_append(MOUNT_POINT "/hello.txt");
    test_teardown();
}
...
{
    test_setup(OPENFILES);
    test_red_lseek(MOUNT_POINT "/seek.txt");
    test_teardown();
}
...
{
    test_setup(OPENFILES);
    test_red_stat(MOUNT_POINT "/stat.txt", MOUNT_POINT "");
    test_teardown();
}
...
{
    test_setup(OPENFILES);
    test_red_unlink(MOUNT_POINT "/unlink.txt");
    test_teardown();
}
...
{
    test_setup(OPENFILES);
    test_red_link_rename(MOUNT_POINT "/link");
    test_teardown();
}
...
{
    test_setup(OPENFILES);
    test_red_mkdir_rmdir(MOUNT_POINT "/dir");
    test_teardown();
}
...
{
    test_setup(OPENFILES);
    test_red_can_opendir(MOUNT_POINT "");
    test_teardown();
}
...
{
    test_setup(OPENFILES);
    test_red_opendir_readdir_rewinddir(MOUNT_POINT "/dir");
    test_teardown();
}
...
{
    test_setup(OPENFILES);
    test_red_concurrent(MOUNT_POINT "/f");
    test_teardown();
}
...
TEST_CASE(can_read_write, "write/read speed test", "[fatfs][wear_levelling]")
{
    ...
    test_red_rw_speed(file, buf, 4 * 1024, file_size, true);
    test_red_rw_speed(file, buf, 8 * 1024, file_size, true);
    test_red_rw_speed(file, buf, 16 * 1024, file_size, true);

    test_red_rw_speed(file, buf, 4 * 1024, file_size, false);
    test_red_rw_speed(file, buf, 8 * 1024, file_size, false);
    test_red_rw_speed(file, buf, 16 * 1024, file_size, false);
