...
#include "ff.h"
#include "test_red_common.h"

const char* red_test_hello_str = "Hello, World!\n";

void test_red_create_file_with_text(const char* name, const char* text)
{
...
void test_red_overwrite_append(const char* filename)
{
    /* Create new file with 'aaaa' */
    test_red_create_file_with_text(filename, "aaaa");
    ...
    test_red_create_file_with_text(filename, "cccc");
...
void test_red_read_file(const char* filename)
{
    FILE* f = fopen(filename, "r");
    TEST_ASSERT_NOT_NULL(f);
    char buf[32] = { 0 };
    int cb = fread(buf, 1, sizeof(buf), f);
    TEST_ASSERT_EQUAL(strlen(red_test_hello_str), cb);
    TEST_ASSERT_EQUAL(0, strcmp(red_test_hello_str, buf));
    TEST_ASSERT_EQUAL(0, fclose(f));
}

void test_red_open_max_files(const char* filename_prefix, size_t files_count)
{
...
void test_red_lseek(const char* filename)
{
...
void test_red_stat(const char* filename, const char* root_dir)
{
    test_red_create_file_with_text(filename, "foo\n");
...
void test_red_unlink(const char* filename)
{
    test_red_create_file_with_text(filename, "unlink\n");
...
void test_red_link_rename(const char* filename_prefix)
{
    char name_copy[64];
    char name_dst[64];
    char name_src[64];
...
void test_red_mkdir_rmdir(const char* filename_prefix)
{
    char name_dir1[64];
    char name_dir2[64];
    ...
    test_red_create_file_with_text(name_dir2_file, "foo\n");
...
void test_red_opendir_readdir_rewinddir(const char* dir_prefix)
{
    ...
    test_red_create_file_with_text(name_dir_file1, "1\n");
    test_red_create_file_with_text(name_dir_file2, "2\n");
    test_red_create_file_with_text(name_dir_file3, "\01\02\03");
    TEST_ASSERT_EQUAL(0, mkdir(name_dir_inner, 0755));
    test_red_create_file_with_text(name_dir_inner_file, "3\n");
...
void test_red_concurrent(const char* filename_prefix)
{
    char names[4][64];
...
void test_red_rw_speed(const char* filename, void* buf, size_t buf_size, size_t file_size, bool write)
{
